...
Research focus has ranged from measurements~\cite{pouwelse2} to protocol
improvements~\cite{bt-impr}, from social networking~\cite{tribler-social} to
moderation techniques~\cite{pouwelse2}, from content distribution
...
libtorrent-rasterbar client~\cite{libtorrent} and a Tribler~\cite{tribler}
client to provide verbose information regarding BitTorrent protocol
implementation. These results are collected
and subsequently processed and analysed through a rendering interface.
...
Client monitoring is enabled trough the use of MonALISA \cite{monalisa}.
MonALISA-specific scripts are used to provide required information to the
central repository. The is typically invoked every 5 seconds. The service
station collects required information and sends them to the monitoring
repository.
...
Figure \ref{fig:monitoring} is an architectural view of the monitoring
infrastructure. The core component of the monitoring framework is
the MonALISA service. The service is able to discover other
monitoring services and to be discovered by interested clients. Each
service registers itself with a set of Lookup Services (LUSs) as part of a
dedicated monitoring group and it publishes a set of dynamic
attributes that describe it. This way any interested application can
request specific services based on a set of matching attributes.
...
hrktorrent/libtorrent~\cite{hrktorrent}~\cite{libtorrent} and Tribler~\cite{tribler}.
...
applications: Tribler~\cite{tribler} and hrktorrent~\cite{hrktorrent} (based
on libtorrent-rasterbar~\cite{libtorrent}). While the latter needed minimal
changes in order to provide the necessary verbose and status data, Tribler had
...
the following protocol messages~\cite{bt}:
\begin{itemize}
...
As shown in Figure~\ref{fig:logarch}, using parsers specific to each type of
logging file, messages are sent as input to the \textit{Database Access}
module that stores them into an SQLite database. In order to analyse peer
behaviour the Rendering Engine reads stored logging data using the Database
...
definition or approximation for Equation \ref{eq:eval}. This will allow
the establishment of the most important units to be considered and updated to
ensure peer and swarm performance.
